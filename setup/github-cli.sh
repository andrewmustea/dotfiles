...
if [[ "${NAME}" == "debian" ]] || [[ "${NAME}" == "ubuntu" ]]; then
  ...
  # download key
  if ! sudo curl -fsSL --create-dirs "${SOURCE}" -o "${KEYRING}"; then
  ...
  echo "deb [arch=${ARCH} signed-by=${KEYRING}] ${REPO} stable main" | sudo tee "${LIST}"

  # update apt package info and install github-cli
  if hash nala &>/dev/null; then
    sudo nala update && sudo nala install -y gh
  else
